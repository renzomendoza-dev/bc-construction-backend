...
-- existing convention).
--
-- current_warehouse_id/warehouse_id/return_warehouse_id are plain BIGINT
-- columns on the Java side (no @ManyToOne - Warehouse lives in the inventory
-- module, a different module from equipment's own entities), but real FKs at
-- the DB level: warehouse already exists (inventory's own V6, well before
-- this migration), so unlike transfer_batch.source_material_request_id
-- (which skips the FK purely for migration ORDER reasons), there's no
-- ordering reason to skip it here.
...
CREATE TABLE equipment_assignment (
    ...
    return_warehouse_id   BIGINT REFERENCES warehouse(id),
