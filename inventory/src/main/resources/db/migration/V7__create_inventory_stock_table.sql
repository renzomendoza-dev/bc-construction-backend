...
CREATE TABLE inventory_stock (
    ...
    -- same item_id+warehouse_id and a NULL location_id (confirmed by the
    -- observed-behavior @DataJpaTest written earlier for this schema:
    -- InventoryStockRepositoryTest.shouldHandleSameItemAndWarehouseWithNullLocationInBothRows).
    -- If "at most one warehouse-level stock row per item+warehouse" needs to
    -- be a hard DB-level invariant, add a partial unique index instead of/
